# Weekly Agent - User Guide

## Overview
The Weekly Agent uses intelligent keyword parsing to automatically create tasks from natural language prompts. Simply describe your tasks naturally, and the agent will extract all the details and add them to the appropriate days.

## How to Use

### 1. Open the Weekly Agent
- Look for the **Weekly Agent** button with the robot icon 🤖
...
### 3. Keywords the Agent Understands
...
The agent removes keywords from the task title, so:
```
...
1. **Always mention the day** - The agent needs to know when to schedule the task
